...
def packed_bytes_to_iq(file_path):
    ...
    print(iq_data.shape)
    iq_data = np.ctypeslib.as_array(iq_data)
    iq_data = iq_data.astype(np.float64).view(np.complex128)

    iq_data /= 127.5
    iq_data -= (1 + 1j)
    iq_data.shape = (72800,)
    return sample_rate, iq_data
...
fs, data = packed_bytes_to_iq("/home/jayaraj/Projects/calibratesdr_sdrtest/GSM/sdr-gsm-recordings/SDRSharp_20210327_124115Z_935800000Hz_IQ.wav")
...
plot(r_[0:72800] / fs * 1000, abs(data[:72800]))
